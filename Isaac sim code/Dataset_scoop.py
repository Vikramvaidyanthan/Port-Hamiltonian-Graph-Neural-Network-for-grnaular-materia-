...
SAVE_DIR          = "/home/rllab_msc_student/Vikram/assets/DatasetTest_scoop"
# ────────────────────────────────────────────────────────
...
async def shovel_sequence():
    try:
        ...
        for loop_iter in range(NUM_LOOPS):
            ...
            # ── One independent x per active (non-flat) phase ──
            x_lower_to_pile = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)
            x_scoop_open    = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)
            x_scoop_close   = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)
            x_straighten    = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)
            x_drop_1        = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)
            x_drop_2        = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)
            x_drop_3        = rng.uniform(-MAX_SH0_DELTA, MAX_SH0_DELTA)

            print(f"\n{'='*60}")
            print(f" LOOP {loop_iter+1}/{NUM_LOOPS}")
            print(f"  lower_to_pile  x = {x_lower_to_pile:+.5f}")
            print(f"  scoop_open     x = {x_scoop_open:+.5f}")
            print(f"  scoop_close    x = {x_scoop_close:+.5f}")
            print(f"  straighten     x = {x_straighten:+.5f}")
            print(f"  drop_1         x = {x_drop_1:+.5f}")
            print(f"  drop_2         x = {x_drop_2:+.5f}")
            print(f"  drop_3         x = {x_drop_3:+.5f}")
            print(f"{'='*60}")

            keyframes = [
                # Phase              Steps  sh0                          sh1      el0     el1     wr0      wr1
                ("flat_start",        30,  {sh0:  0.0,                  sh1: -1.225, el0: 1.2,   el1: 0.375, wr0: -0.192, wr1: 0}),
                ("lower_to_pile",     30,  {sh0:  0.0 + x_lower_to_pile, sh1: -0.8,  el0: 1.7,   el1: 0.375, wr0:  0.192, wr1: 0}),
                ("scoop_open",        30,  {sh0:  0.0 + x_scoop_open,   sh1: -0.4,  el0: 1.3,   el1: 0.375, wr0:  0.192, wr1: 0}),
                ("scoop_close",       30,  {sh0:  0.0 + x_scoop_close,  sh1: -0.8,  el0: 1.0,   el1: 0.375, wr0:  0.192, wr1: 0}),
                ("straighten",        30,  {sh0:  0.0 + x_straighten,   sh1: -1.225,el0: 0.836, el1: 0.375, wr0:  0.192, wr1: 0}),
                ("drop_1",            30,  {sh0:  0.0 + x_drop_1,       sh1: -1.225,el0: 0.836, el1: 0.375, wr0:  0.75,  wr1: 0}),
                ("drop_2",            30,  {sh0:  0.0 + x_drop_2,       sh1: -1.225,el0: 0.836, el1: 0.375, wr0:  0.75,  wr1: 3.14}),
                ("drop_3",            30,  {sh0:  0.0 + x_drop_3,       sh1: -1.225,el0: 0.836, el1: 0.375, wr0:  1.2,   wr1: 3.14}),
            ]

            # Map each phase name → its x value for logging in dataset
            phase_x_map = {
                "flat_start":    0.0,
                "lower_to_pile": x_lower_to_pile,
                "scoop_open":    x_scoop_open,
                "scoop_close":   x_scoop_close,
                "straighten":    x_straighten,
                "drop_1":        x_drop_1,
                "drop_2":        x_drop_2,
                "drop_3":        x_drop_3,
            }
